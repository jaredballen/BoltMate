...
internal static class Commands
{
    ...
    {
        Console.WriteLine("boltmate — companion switcher for Logitech Bolt receivers");
        Console.WriteLine();
        Console.WriteLine("Usage:");
        Console.WriteLine("  boltmate list                       List receivers + paired devices (default if no command).");
        Console.WriteLine("  boltmate monitor                    Listen for Easy-Switch / Flow events and fan out.");
        Console.WriteLine("  boltmate switch <host>              Switch ALL paired devices to host (0..2).");
        Console.WriteLine("  boltmate device <slot> switch <host>  Switch a single slot (1..6) to host (0..2).");
        Console.WriteLine("  boltmate device <slot> unpair         Unpair slot (1..6) from the first receiver. Destructive.");
        Console.WriteLine("  boltmate device --receiver <idx> <slot> unpair");
        Console.WriteLine("  boltmate device <slot> rename <name>  Rename a paired device. Currently rejected by firmware (see #32).");
        Console.WriteLine("  boltmate receiver clear [--yes]       Unpair every device on the first receiver. Destructive.");
        Console.WriteLine("  boltmate backup [path]                Dump all receiver pairings to JSON.");
        Console.WriteLine("  boltmate diagnose [path]              Bundle pairings + logs + system info into a zip.");
        Console.WriteLine("  boltmate diag                       Dump every raw HID++ frame on the wire.");
        Console.WriteLine("  boltmate service install            Register as a background service / login agent.");
        Console.WriteLine("  boltmate service uninstall          Remove the background registration.");
        Console.WriteLine("  boltmate service status             Show service registration status.");
        Console.WriteLine("  boltmate tail [-n<N>] [--app|--cli] [--no-follow]");
        Console.WriteLine("                                      Tail the newest log file (App by default). Survives log roll.");
        Console.WriteLine("  boltmate diag-divert <slot> <cid>   Probe Set/Clear divert on a single CID (engineering tool).");
        Console.WriteLine("  boltmate probe-host-bindings [slot] Sweep 0x1815 fns for writeable host-identifier surfaces (all linked slots by default).");
        Console.WriteLine("  boltmate help                       This message.");
    }
    ...
    public static async Task<int> RunDiagnoseAsync(IReceiverTransport transport, string? outputPath, CancellationToken ct)
    {
        ...
        var tmpDir = Directory.CreateTempSubdirectory("boltmate-diag-");
        try
        {
    ...
    public static async Task<int> RunSniffAllInterfacesAsync(CancellationToken ct)
    {
        ...
        var manager = BoltMate.Hid.IOKit.IOKitInterop.IOHIDManagerCreate(IntPtr.Zero, 0);
        if (manager == IntPtr.Zero) { logger.LogError("IOHIDManagerCreate failed"); return 1; }

        try
        {
            var match = BoltMate.Hid.IOKit.IOKitInterop.CreateMatchingDictionary(0x046D, 0xC548);
            BoltMate.Hid.IOKit.IOKitInterop.IOHIDManagerSetDeviceMatching(manager, match);
            BoltMate.Hid.IOKit.IOKitInterop.CFRelease(match);

            BoltMate.Hid.IOKit.IOKitInterop.IOHIDManagerOpen(manager,
                BoltMate.Hid.IOKit.IOKitInterop.OptionsNone);

            var set = BoltMate.Hid.IOKit.IOKitInterop.IOHIDManagerCopyDevices(manager);
            if (set == IntPtr.Zero) { logger.LogError("No Bolt interfaces found"); return 1; }

            var count = (int)BoltMate.Hid.IOKit.IOKitInterop.CFSetGetCount(set);
            var devices = new IntPtr[count];
            BoltMate.Hid.IOKit.IOKitInterop.CFSetGetValues(set, devices);
            ...
            var callbacks = new List<BoltMate.Hid.IOKit.IOKitInterop.IOHIDReportCallback>();
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var usagePage = BoltMate.Hid.IOKit.IOKitInterop.GetInt32Property(dev, "PrimaryUsagePage") ?? 0;
                var usage = BoltMate.Hid.IOKit.IOKitInterop.GetInt32Property(dev, "PrimaryUsage") ?? 0;
                var product = BoltMate.Hid.IOKit.IOKitInterop.GetStringProperty(dev, "Product") ?? "?";
                var label = $"iface#{i} UP=0x{usagePage:X4} U=0x{usage:X4} ({product})";

                var openResult = BoltMate.Hid.IOKit.IOKitInterop.IOHIDDeviceOpen(dev,
                    BoltMate.Hid.IOKit.IOKitInterop.OptionsNone);
                if (openResult != 0)
                {
                ...
                BoltMate.Hid.IOKit.IOKitInterop.IOHIDReportCallback cb =
                ...
                callbacks.Add(cb);
                BoltMate.Hid.IOKit.IOKitInterop.IOHIDDeviceRegisterInputReportCallback(
            ...
            {
                var rl = BoltMate.Hid.IOKit.IOKitInterop.CFRunLoopGetCurrent();
                foreach (var dev in openedDevices)
                    BoltMate.Hid.IOKit.IOKitInterop.IOHIDDeviceScheduleWithRunLoop(dev, rl,
                        BoltMate.Hid.IOKit.IOKitInterop.CFRunLoopDefaultMode);
                BoltMate.Hid.IOKit.IOKitInterop.CFRunLoopRun();
            }) { IsBackground = true };
            thread.Start();
            ...
            foreach (var dev in openedDevices)
                BoltMate.Hid.IOKit.IOKitInterop.IOHIDDeviceClose(dev,
                    BoltMate.Hid.IOKit.IOKitInterop.OptionsNone);
            foreach (var buf in buffers) Marshal.FreeHGlobal(buf);
            BoltMate.Hid.IOKit.IOKitInterop.CFRelease(set);
            return 0;
        }
        finally
        {
            BoltMate.Hid.IOKit.IOKitInterop.IOHIDManagerClose(manager,
                BoltMate.Hid.IOKit.IOKitInterop.OptionsNone);
            BoltMate.Hid.IOKit.IOKitInterop.CFRelease(manager);
        }
    }
    ...
    public static async Task<int> RunDiagIOKitOpenAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var open = BoltMate.Hid.IOKit.IOKitInterop.IOHIDManagerOpen(manager, BoltMate.Hid.IOKit.IOKitInterop.OptionsNone);
            ...
            {
                if (dev == IntPtr.Zero) continue;
                var usagePage = BoltMate.Hid.IOKit.IOKitInterop.GetInt32Property(dev, "PrimaryUsagePage");
                var usage = BoltMate.Hid.IOKit.IOKitInterop.GetInt32Property(dev, "PrimaryUsage");
            ...
            var openResult = BoltMate.Hid.IOKit.IOKitInterop.IOHIDDeviceOpen(
                managementDevice, BoltMate.Hid.IOKit.IOKitInterop.OptionsNone);
            ...
            BoltMate.Hid.IOKit.IOKitInterop.IOHIDDeviceClose(managementDevice, BoltMate.Hid.IOKit.IOKitInterop.OptionsNone);
            BoltMate.Hid.IOKit.IOKitInterop.CFRelease(set);
        ...
        {
            BoltMate.Hid.IOKit.IOKitInterop.IOHIDManagerClose(manager, BoltMate.Hid.IOKit.IOKitInterop.OptionsNone);
            BoltMate.Hid.IOKit.IOKitInterop.CFRelease(manager);
        }
    }
    ...
    /// <summary>
    /// Tails the newest BoltMate log file matching the given prefix.
    /// App writes boltmate-app-*.log; CLI writes boltmate-*.log. Both land
    /// in <see cref="AppPaths.LogsDirectory"/>, so one tail can target either
    /// (or both) by prefix glob.
    /// </summary>
    /// <param name="prefix">File prefix glob, e.g. "boltmate-app-*.log" for
    /// the App, "boltmate-*.log" for everything. Defaults to the App if any
    ...
    public static async Task<int> RunTailAsync(int lastN, bool follow, string? prefix, CancellationToken ct)
    {
        ...
        string ResolvePrefix()
        {
            ...
            var hasAppLog = new DirectoryInfo(dir).GetFiles("boltmate-app-*.log").Any();
            return hasAppLog ? "boltmate-app-*.log" : "*.log";
        }
