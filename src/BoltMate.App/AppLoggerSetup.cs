...
internal static class AppLoggerSetup
{
    ...
    {
        // Default level resolution: env var override first (BOLTMATE_LOG_LEVEL=Debug),
        // then the caller-supplied value, then Information.
        var envOverride = Environment.GetEnvironmentVariable("BOLTMATE_LOG_LEVEL");
        var effective =
    ...
    private static string ResolveLogsDirectory()
    {
        if (OperatingSystem.IsMacOS())
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Library", "Logs", "BoltMate");
        if (OperatingSystem.IsWindows())
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "BoltMate", "Logs");
        var xdg = Environment.GetEnvironmentVariable("XDG_STATE_HOME");
        return !string.IsNullOrEmpty(xdg)
            ? Path.Combine(xdg, "BoltMate")
            : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".local", "state", "BoltMate");
    }
